...
export class Store {
  ...
  fetch() {
    this.git(['fetch', this.remote, `+refs/heads/${this.branch}:${this.remoteRef}`], { acceptCodes: [128] });
  }
  ...
  commit({ mutate, message, attempts = 4 }) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      const out = mutate(state ? structuredClone(state) : null, this.readJournalTail());
      if (out.noop) return { committed: false, reason: out.reason, state };
      ...
      try {
        ...
        if (push.status === 0) {
          return { committed: true, sha: commit, state: out.state };
        }
